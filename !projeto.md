...
> **Redesign visual (2026-07-07 — tarefas 10-14 do `!executar.md`, plano do Gemini em `gemini-design.md`):** Design System v2 em `css/redesign.css` (global via topo.php); **cor única por usuário** em toda a plataforma (`window.corDoUsuario`/`chipUsuarioHtml`/`avatarUsuarioHtml` no `painel.js`); **Canais** virou LISTA rica (`#listaAtividades`, linhas `.canal-linha`) com switch binário Ativado/Desativado (`aberta`↔`cancelada`; legado em_andamento/concluida só é convertido quando o admin mexe), filtros de status/usuário/busca, toggle "Mostrar adm" e destaque "Sem vínculos"; **Dashboard** em 2 colunas (lg-8 gráficos/declarado + lg-4 "Visão Geral da Equipe" com métricas mini e lista viva `#listaMembrosVivo` — a tabela `#tbodyResumoUsuariosGraficos` NÃO existe mais); **Gestão do Usuário** em 4 nav-pills (Resumo & Canais / Tarefas / Pagamentos & Credenciais / Auditoria — todos os ids preservados, `shown.bs.tab` dispara resize pros ECharts); **login** com vidro + gradiente RK. Backend novo: `atividades/excluir.php` também bloqueia exclusão de canal com conteúdo no MEGA.
>
