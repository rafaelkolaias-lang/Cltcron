...
| `CronometroLeve.spec` | Spec do PyInstaller. Entry = `main.py`. Usa `collect_all('nacl')` para garantir `_sodium.pyd` + `libsodium.dll` (pynacl) no `.exe`. `hiddenimports` lista todos os módulos de `app/` (incluir aqui ao adicionar módulo novo). |
...
| `pyproject.toml` | Config de tooling: `ruff` (target py310, line-length 120), `mypy`, `pytest` (testpaths `tests/`). **`version = "2.2"` está desatualizado** vs `VERSAO_APLICACAO=v3.0` em `app/config.py` — fonte da verdade da versão é `app/config.py`. |
...
- **Sanitização de nomes de pasta MEGA (v4.0+):** dupla — PHP (`mega_normalizar_nome_pasta` em `_comum.php`) remove `"<>|?*:` na criação; Python (`_sanitizar_caminho_mega` em `mega_uploader.py`) sanitiza todos os args de `_run_mega` + `pasta_remota` em `upload_arquivo` na execução. Aspas duplas viram apóstrofo, demais proibidos são removidos.
...
| `mega.php` | **(Parte 7)** Página dedicada do MEGA (3 blocos: pastas lógicas, config por canal, campos por usuário+canal). Edição inline, sem modais. `js/aba-mega.js` (com auto-init standalone) foi movido inteiro — sem acoplamento, removido do index. |
...
mega/                          — Módulo MEGA (Fase 1 — config admin + endpoints desktop). Tabelas auto-criadas
                                 ...
                                 _estrutura.php             — DDL idempotente (CREATE TABLE IF NOT EXISTS) das 4
                                 ...
                                 _comum.php                 — Helpers: mega_normalizar_nome_pasta($num,$titulo)
                                                              produz "NN - Titulo" canônico; mega_normalizar_extensoes()
                                 ...
                                 campos_listar.php          — GET (?user_id=X&id_atividade=Y&incluir_inativos=1) campos
                                                              de upload exigidos.
                                 campos_salvar.php          — POST upsert UM campo (id_campo opcional).
...
| `cronometro_relatorios` | Relatórios finalizados | **`referencia_data` = data REAL do trabalho** (não `criado_em`). Fonte do Resumo |
...
| `mega_campos_upload` | **(Fase 1 MEGA)** Campos de upload exigidos por `user_id + id_atividade`. | Sem UNIQUE composto (admin pode ter vários campos por par). Soft-delete (`ativo`). Colunas: `label_campo`, `extensoes_permitidas` (CSV `mp4,zip`), `quantidade_maxima`, `obrigatorio`, `ordem`. |
