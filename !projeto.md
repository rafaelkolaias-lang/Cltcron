...
- **Guarda "salvei mas não declarei" (v4.1.1+):** no `salvar()` do form MEGA (`app/subtarefas.py`, logo após calcular `deve_concluir`), ao EDITAR uma sub ainda Aberta (`subtarefa is not None and not subtarefa_concluida`) em modo manual (`not auto_apos_upload`) e SEM tempo válido (`not deve_concluir`), aparece `messagebox.askyesno("Tarefa NÃO declarada", "Você não preencheu o campo TEMPO GASTO. Continuar mesmo assim?")`. "Não" volta ao form; "Sim" salva como Aberta (preserva edição só-observação). Corrige o bug do Marcus: ele digitava o tempo no campo Observação, o botão ficava "Salvar" (não "Salvar e Concluir"), o app gravava só a observação via `atualizar_subtarefa` e fechava em silêncio — `concluir_subtarefa` nunca rodava, sub seguia Aberta com `segundos_gastos=0`. Não bloqueia: auto-save após upload, sub nova e sub concluída não disparam o aviso.
