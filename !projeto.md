...
| `mega-campos.php` | Página **MEGA · Campos de upload** (split de 2026-06-05). Ordem: campos por usuário+canal (com `<select>` de Tipo + botão **"Usar modelo existente"** → modal `#modalUsarModelos` com checkboxes que adiciona vários modelos de uma vez) → **tabela CRUD de Modelos inline** (`#tbodyMegaModelos`: editar/excluir/+novo, substituiu os popups `window.prompt`) → **Configuração por canal** (no fim). Link "← Pastas lógicas" no topo. Tem `<section id="abaMega">`. Acessada pelo submenu de "MEGA" no `_layout/topo.php`. |
