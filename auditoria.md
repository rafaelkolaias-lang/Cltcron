...
#### 34. 🔴 Crítico (10) — Sessão que atravessa dias (restaurada ao reabrir o app) PERDE todas as horas cronometradas — relatórios ficam zerados no banco

- **QUANDO ACONTECE:** quando o usuário NÃO zera o cronômetro no fim do dia — fecha o app com a sessão aberta e no dia seguinte ela é **restaurada automaticamente** ("SESSÃO RESTAURADA (PAUSADA)"). Ele retoma, trabalha normalmente (o cronômetro conta na tela, heartbeats chegam ao banco a cada minuto), mas as linhas de horas em `cronometro_relatorios` daquela sessão ficam **0** em todos os dias e permanecem 0 até o fim. Quando ele finalmente clica em "Zerar Cronômetro" (que promete "as horas ficam salvas no banco"), nada é gravado — todas as horas da sessão somem.
- **ONDE (evidência em produção, leitura 2026-07-07):** sessões multi-dia encerradas: alex #277/281/285/287/291, joao #276/279/292, marcus #280/286 — **todas com `SUM(segundos_trabalhando)=0`** apesar de 271–920 heartbeats "trabalhando" cada. Sessões iniciadas e encerradas com fluxo "normal" (ex.: alex #283 19h53, joao #288 12h26, #289 10h05) gravaram. Estimativa de perda desde 16/06: **alex ~50h, joao ~20h, marcus ~12h (~85h da equipe)**.
- **SEVERIDADE:** 🔴 Crítico 10 — perda permanente de horas trabalhadas (impacto financeiro direto: o usuário não consegue declarar/receber).
- **IMPACTO:** o usuário cronometra o dia inteiro e o sistema "esquece"; o saldo declarável não sobe; reclamações tipo "cronometrei e só computou 2h" (alex: as únicas horas dele na semana são 2h36 de 03/07 + o dia atual, ambas de sessões novas do próprio dia).
- **DETALHE TÉCNICO (o que se sabe):** as linhas zeradas têm `relatorio='Sessão em andamento (parcial)'` — o último escritor foi o flush parcial (`app/monitor.py::_upsert_relatorio_parcial` → `_upsert_relatorio_com_snapshots`, upsert por `(id_sessao, referencia_data)` com rateio `dividir_tempos_por_dia`), gravando snapshot **zerado** repetidamente; o upsert do `zerar_sessao()` (texto "Sessão zerada") **nunca chegou a escrever** nessas sessões. Nas sessões preservadas o texto é "Sessão zerada" com horas reais. Ou seja: nos ciclos com restauração, os acumuladores em memória aparentam estar **zerados** durante toda a sessão restaurada (apesar de `restaurar_sessao` ler `segundos_trabalhando` do arquivo local) e cada flush de 5min/pausa **sobrescreve as linhas de TODOS os dias da sessão com 0**. Causa-raiz exata (por que o acumulador/arquivo está zerado após a restauração — possivelmente estado local salvo zerado em algum fluxo de fechamento/atualização, ou snapshot capturado de instância errada) **ainda não confirmada em código** — requer reprodução local com log técnico (`LOG_TEC` grava `relatorio UPDATE/INSERT` com valores no `.cronometro_leve_log.json` da máquina do usuário; pedir esse arquivo ao alex aceleraria o diagnóstico). Relacionado ao #14 (rateio por dia de relógio), mas é mecanismo distinto: aqui o valor gravado é 0.
- **DADOS RECUPERÁVEIS:** os heartbeats (`cronometro_eventos_status`) e `cronometro_foco_janela`/`cronometro_apps_intervalos` preservam o rastro real minuto a minuto — dá pra reconstruir as horas perdidas por dia e lançar correções manuais em `cronometro_relatorios` (mesmo precedente Marcus/Eliseu/joao do `reminder.md`), mediante autorização.
- **STATUS (2026-07-08, Claude 1 — autorizado pelo usuário):**
  - **Código CORRIGIDO e DISTRIBUÍDO** em `app/monitor.py` (commit `18d4908`, **release v4.1.4 de 08/07/2026** no GitHub + auto-update ativo — clientes atualizam no próximo login): (a) acumulação POR DIA (`_tempos_por_dia`, persistida no estado local e restaurada com a sessão) — cada dia grava exatamente o que foi trabalhado nele, fim do rateio retroativo (resolve também o #14 para dados novos); (b) **UPDATE monotônico com GREATEST** — horas de um dia nunca diminuem, flush/zerar com acumulador zerado não consegue mais apagar nada; (c) try/except por fatia (falha em um dia não pula os outros) + SELECT com ORDER BY determinístico. Validado com repro (`scratchpad/repro_bug34.py`: dias corretos, ataque com acumulador zerado não rebaixa) + pytest (79 pass; 3 falhas pré-existentes sem relação) + ruff limpo.
  - **HORAS RECUPERADAS em produção** (UPDATE nas linhas zeradas, reconstrução conservadora por heartbeats, limitada ao déficit real de cada dia, criado_em=NOW p/ contar no ciclo atual): **alex +50,0h, joao +16,3h, marcus +15,9h = 82,3h** (27 linhas, sessões 274-292 fechadas).
  - **v4.1.4 já publicada (auto-update)** — até cada membro relogar no app e atualizar, vale o paliativo de zerar o cronômetro no fim do dia. Sessões abertas (282/293/294) não foram tocadas na recuperação.
  - **PENDENTE DE DECISÃO DO USUÁRIO — perdas ANTERIORES a 12/06 não recuperadas:** a mesma reconstrução revela déficits grandes do incidente #33 (gravação travada 26/05→11/06, corrigida sem backfill) e de abril/maio: aprox. alex +81h, joao +76h (dos quais 16h já compensadas via correção manual id 367), marcus +16h (6h já compensadas via id 288), richard +33h (abril), eliseu/clara/deivid pequenos. Interage com ciclos já pagos e com as correções manuais do `reminder.md` — decidir escopo antes de aplicar.
...
#### 34. 🔴 Crítico (9) — Conta ADM: ao "Declarar Tarefa", o arquivo selecionado (ex.: thumb) NÃO subia pro MEGA, mas a tarefa salvava como se tivesse subido — CORRIGIDO 2026-06-12 (Claude 1)

- **QUANDO ACONTECIA:** sempre que a conta **adm** selecionava um arquivo num campo do formulário "Declarar Tarefa" (MEGA) e clicava em **Salvar / Salvar e Concluir**. O arquivo ficava só "selecionado" na tela, a **tela de envio (barra de progresso) nunca aparecia**, nada ia pro MEGA e nenhum registro entrava em "Upado por". Mesmo assim a tarefa era gravada — dando a falsa impressão de que o arquivo subiu. Reproduzido pelo usuário: canal "Novo canal NIU", pasta "09 - Por que é impossível viajar para saturno", thumb não chegou ao MEGA e "Upado por" ficou só com `alex`, sem `adm`.
- **ONDE:** `app/subtarefas.py::_atualizar_botao_salvar` e `salvar()`. O botão só entrava no modo **"Enviar Arquivos"** (que dispara `_abrir_popup_envios()` → upload real) quando havia campo **obrigatório pendente** (`obrig_pendente`). Com todos os campos opcionais, caía direto em "Salvar"/"Salvar e Concluir" e `salvar()` pulava o `_abrir_popup_envios()`, gravando a subtarefa **sem subir os arquivos selecionados** (que ficavam só com `state="pendente"` + `arquivo_local`, nunca enviados).
- **POR QUE ERA ESPECÍFICO DA ADM:** desde o commit `361fa0f` (2026-06-11, "conta adm recebe todos os modelos de campo… obrigatorio sempre false"), `desktop_obter_config.php` devolve **todos os campos da adm com `obrigatorio=false`**. Logo, pra adm `obrig_pendente` era **sempre vazio** → o gatilho "Enviar Arquivos" **nunca** ligava → a adm **nunca** conseguia subir arquivo por esse formulário. Usuários comuns também eram atingidos em campos **opcionais** quando não havia nenhum obrigatório pendente segurando o botão em "Enviar Arquivos".
- **SEVERIDADE:** 🔴 Crítico 9 — ação importante (subir thumb/projeto) "parecia funcionar mas não persistia" nem chegava ao MEGA; o painel marcava a tarefa como entregue sem o arquivo existir. Pra adm era 100% reprodutível.
- **Solução aplicada (2026-06-12, autorizada pelo usuário):** em `_atualizar_botao_salvar`, após o bloco de obrigatórios, adicionado um check de **arquivos staged pendentes** (`state != "concluido"` E `arquivo_local` preenchido, para QUALQUER campo, obrigatório ou não). Havendo staged pendente, força `var_texto_botao="Enviar Arquivos"` e retorna — o `salvar()` então dispara `_abrir_popup_envios()` (sobe os arquivos e salva a tarefa sozinho no fim). O tempo digitado é preservado pelas `StringVar` mesmo após o `janela.destroy()`. O botão "Salvar/Salvar e Concluir" foi **mantido** (necessário pra concluir tarefa pós-upload, tarefa só-pasta e edição sem trocar arquivo) — ele agora só aparece quando NÃO há arquivo pendente, que é o comportamento correto. **Desdobramento (mesmo dia):** rotear o auto-save após upload expôs um crash quando a thumb subia SEM tempo válido — o auto-save forçava `deve_concluir=True` → `_converter_texto_tempo_para_segundos` levantava "Informe um tempo maior que zero", e o `messagebox.showerror(parent=janela)` quebrava (`bad window path name`) porque a janela já fora destruída na etapa "Enviar Arquivos". Fix em `salvar()`: no auto-save após upload, tempo inválido/zero NÃO quebra nem perde o upload — a tarefa é salva como **Aberta** (`deve_concluir = segundos_tempo > 0`), e o messagebox de erro ganhou fallback `parent=self`. Caso típico: conta adm entregando só a thumb, sem declarar horas. `py_compile` OK.
