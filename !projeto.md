...
| `app/monitor.py` | `EstadoMonitor` (dataclass) + `MonitorDeUso`. Coração do cronômetro: sessão, heartbeat, fila offline, foco/apps, persistência local, snapshots, integração com `DetectorInputSintetico`. **Persistência de horas (bug #34, 2026-07-08):** acumulação POR DIA em `_tempos_por_dia` (dict `{iso_date: {trab,oci,pau}}`, salvo no estado local e restaurado com a sessão) — o upsert de `cronometro_relatorios` grava cada dia com o que foi trabalhado NELE (o rateio `dividir_tempos_por_dia` virou fallback legado); UPDATE **monotônico** (`GREATEST`) — horas de um dia nunca diminuem no banco; try/except por fatia. NÃO remover: é a defesa contra perda de horas em sessões multi-dia restauradas. |
...
| `banco.py` | Camada PDO MySQL thread-safe. Editar só ao mudar conexão. **Timeouts obrigatórios (v4.1.5+):** `TIMEOUT_CONEXAO_SEGUNDOS=8` / `TIMEOUT_LEITURA_SEGUNDOS=30` / `TIMEOUT_ESCRITA_SEGUNDOS=30` passados ao `pymysql.connect()`. **NÃO remover:** o `connect_timeout` cobre só o handshake TCP — logo depois o pymysql faz `sock.settimeout(None)`, então sem `read_timeout`/`write_timeout` qualquer leitura bloqueia PARA SEMPRE quando a conexão abre mas o servidor não responde (firewall/antivírus segurando a 3306, rota ruim do provedor). Era a causa do login travado em "Verificando…" eternamente. `read_timeout` vale pra TODAS as queries do app — se alguma consulta legítima passar de 30s, subir o valor, nunca zerar. |
...
| `pyproject.toml` | Config de tooling: `ruff` (target py310, line-length 120), `mypy`, `pytest` (testpaths `tests/`). **`version = "2.2"` está desatualizado** vs `VERSAO_APLICACAO` em `app/config.py` (atualmente v4.1.4) — fonte da verdade da versão é sempre `app/config.py`. |
...
- **Bug "declarei mas não concluiu" — causa raiz (v4.1.2):** no `salvar()` do form MEGA (`app/subtarefas.py`), o `deve_concluir` era calculado lendo `var_texto_botao.get() == "Salvar e Concluir"` DEPOIS de a linha `var_texto_botao.set("Salvando…")` já ter trocado o texto → `deve_concluir` dava **sempre False** no clique manual. Resultado: por mais que o usuário preenchesse o TEMPO GASTO, o app só rodava `atualizar_subtarefa` (sem `segundos_gastos`), NUNCA `concluir_subtarefa`, e fechava sem erro — a sub ficava Aberta com `segundos_gastos=0` (bug do Marcus). **Fix:** capturar `intencao_concluir = var_texto_botao.get() == "Salvar e Concluir"` ANTES do `set("Salvando…")` e usar essa flag no `deve_concluir` (o form legado já fazia certo — capturava antes).
- **Guarda "salvei mas não declarei" (v4.1.1+):** ainda no `salvar()` do form MEGA, ao EDITAR uma sub ainda Aberta (`subtarefa is not None and not subtarefa_concluida`) em modo manual (`not auto_apos_upload`) e SEM tempo válido (`not deve_concluir`), aparece `messagebox.askyesno("Tarefa NÃO declarada", "Você não preencheu o campo TEMPO GASTO. Continuar mesmo assim?")`. "Não" volta ao form; "Sim" salva como Aberta (preserva edição só-observação). **Depende do fix v4.1.2 acima** — antes dele a guarda disparava SEMPRE (mesmo com tempo preenchido), pois `deve_concluir` era sempre False. Não dispara em: auto-save após upload, sub nova e sub concluída.
- **Validação de arquivo vazio/corrompido no upload (v4.0.8+):** na seleção de arquivo/pasta (handler dos botões de campo `_fazer_handler` E `_iniciar_upload_mega`, ambos em `app/subtarefas.py`), arquivo com `st_size == 0` — ou pasta cujos arquivos somam 0 bytes (`total`/`tamanho_total == 0`) — bloqueia com `messagebox` "Seu arquivo está vazio ou corrompido" e dá `return` antes de qualquer envio ao MEGA. Convive com as validações de pasta vazia (`count == 0`) e extensão.
- **Vídeo publicado = pasta fechada (v4.0.9+):** ao marcar "Publicar" no painel MEGA (`pasta_logica_marcar_publicado.php` grava `mega_pasta_logica.video_publicado=1` — sem mudança de schema), a pasta passa a: (a) aparecer **verde** com "✅ publicado" na lista "Selecionar existente" do app (inclusive pros thumbmakers, que assim sabem que não precisam fazer thumb); (b) **recusar upload** de arquivo novo; (c) **não poder ser excluída**; (d) permitir **só** alterar hora declarada e descrição. **Associação subtarefa↔pasta** = `id_atividade + titulo == nome_pasta`. **Pontos de enforcement:** endpoints expõem `video_publicado` (`mega/desktop_obter_config.php`, `mega/desktop_obter_dados_subtarefa.php`, `atividades_subtarefas/listar.php`); upload barrado em `mega/desktop_registrar_upload.php` (409) + UI `app/subtarefas.py::_iniciar_upload_mega`; exclusão barrada em `declaracoes_dia.py::excluir_subtarefa` (helper `_pasta_logica_publicada`) + checagem antecipada no `_excluir_op` do app (antes de mexer no MEGA); edição restrita a hora+descrição em `atividades_subtarefas/editar.php` + trava de campos no modal `js/aba-gerenciar-tarefas.js`. Verde da lista em `app/subtarefas.py::_inserir_item_pasta`.
...
- **Pasta de tarefa PAGA = somente leitura (2026-07-17, precisa build novo do .exe):** ao selecionar no form MEGA uma pasta com `status_visual='paga'` (subtarefa do próprio user travada por pagamento), `_ao_selecionar_pasta` NÃO reverte mais a seleção — libera o **download** (carrega `_carregar_status_pasta`) e trava o resto: holder `pasta_somente_leitura` + `_aplicar_lock_somente_leitura()` desabilita os botões Selecionar/Trocar/📁 Pasta, `_atualizar_botao_salvar` mostra "Tarefa já paga" e trava o salvar, guard extra no `_handler` de campo e (defense) no fluxo de upload. Trocar pra "Criar nova" ou criar pasta reseta o lock. Antes o clique só mostrava "já foi paga" e o download nem carregava.
...
- Login salvo: `_ler_login_salvo()`, `_salvar_login()`, `_logar()`. **Não existe mais `_tentar_auto_login`/`_mostrar_login_com_erro`/`_montar_tela_carregando`** (removidos em v4.1.5 — eram código morto e um 2º caminho de login sem as proteções novas). O auto-login é o próprio `_logar()` chamado no `__init__`.
- **Login travado em "Verificando…" para sempre (corrigido em v4.1.5):** causa raiz era a falta de `read_timeout` no `banco.py` (ver descrição do arquivo acima). Junto veio a blindagem da tela em `_logar()`: trava `_login_em_andamento` (cliques/Enter repetidos não empilham threads), botão Entrar desabilitado durante a verificação (`self._btn_entrar` + `_definir_estado_btn_entrar`), status `"Verificando… (tentativa N de 3)"`, helper `_liberar_tela()` usado em TODOS os caminhos de saída e `except` de rede de segurança. O retry de 3 tentativas desiste quando passa de `LIMITE_RETRY_LOGIN_SEGUNDOS` (`app/config.py`, 15s) — sem isso o pior caso era 3 × 30s de tela parada. **Armadilha de entrega:** a checagem de update só roda DEPOIS do login, então usuário travado no login nunca recebe auto-update — correção nessa área exige download manual do `.exe`.
...
> **Redesign visual (2026-07-07 — tarefas 10-14 do `!executar.md`, plano do Gemini em `gemini-design.md`):** Design System v2 em `css/redesign.css` (global via topo.php); **cor única por usuário** em toda a plataforma (`window.corDoUsuario`/`chipUsuarioHtml`/`avatarUsuarioHtml` no `painel.js`); **Canais** virou LISTA rica (`#listaAtividades`, linhas `.canal-linha`) com switch binário Ativado/Desativado (`aberta`↔`cancelada`; legado em_andamento/concluida só é convertido quando o admin mexe), filtros de status/usuário/busca, toggle "Mostrar adm", legenda de chips clicável pra mostrar/ocultar cada usuário (padrão: todos visíveis; persistido em localStorage `canais_usuarios_ocultos`; só esconde chips — "Sem vínculos" segue os vínculos reais) e destaque "Sem vínculos"; **Dashboard** em 2 colunas de MESMA altura (lg-8 Monitoramento + lg-4 "Visão Geral da Equipe" com métricas mini e lista viva `#listaMembrosVivo` que rola por dentro — a tabela `#tbodyResumoUsuariosGraficos` NÃO existe mais) e "Tempo Declarado" full-width abaixo da row; **Usuários** também é lista rica no modelo Canais (`#listaUsuarios`, filtro de status + busca — a tabela `#tbodyUsuarios` NÃO existe mais); **Gestão do Usuário** em 5 nav-pills (Resumo & Canais / Tarefas / Pagamentos & Credenciais / Dados do usuário / Auditoria — todos os ids preservados, `shown.bs.tab` dispara resize pros ECharts); **login** com vidro + gradiente RK. Backend novo: `atividades/excluir.php` também bloqueia exclusão de canal com conteúdo no MEGA.
>
...
| `css/painel.css` | Tema RK Produções dark (base legada). |
| `css/redesign.css` | **Design System v2 (redesign 2026-07-07, tarefas 10-14 do `!executar.md`).** Carregado DEPOIS do painel.css em `_layout/topo.php` — sobrescreve/estende em todas as páginas. Variáveis semânticas, cards, forms, tabelas, badges-pílula, `.chip-usuario`/`.avatar-usuario` (cor única por user via `window.corDoUsuario` no painel.js), `.dot-status--*`, `.canal-card` (grid da página Canais), `.membro-vivo` (lista viva do Dashboard), `.mega-*` (página MEGA campos). |
...
pagamentos/
  ...
  desconto_criar.php / desconto_editar.php / desconto_excluir.php — (2026-07-17) CRUD de DESCONTOS avulsos
                          (tabela pagamento_descontos). Desconto em horas × valor_hora do user → R$ que ABATE
                          do "A pagar", MAS não conta como pago e NÃO trava tarefa nem reseta ciclo (por isso
                          tabela própria, fora de Pagamentos — apps desktop antigos leem MAX(data_pagamento) de
                          Pagamentos pra detectar ciclo novo). _descontos.php = garantir_tabela + buscar_usuario.
                          listar_por_usuario.php mescla descontos com tipo='desconto' (sem chave data_pagamento,
                          então Dashboard que soma por data ignora). listar.php (resumo) e relatorio/tempo_
                          trabalhado.php subtraem total_descontos do pendente/A pagar.
...
mega/                          — Módulo MEGA (Fase 1 — config admin + endpoints desktop). Tabelas auto-criadas
                                 ...
                                 pasta_logica_marcar_publicado.php — POST {id_pasta_logica, publicado:0|1}.
                                                              ...
                                                              Registra no log de atividades. Recusa publicar
                                                              vídeo cancelado (409).
                                 pasta_logica_marcar_cancelado.php — POST {id_pasta_logica, cancelado:0|1, nota?}.
                                                              (2026-07-17) Marca/desmarca vídeo como CANCELADO
                                                              (não será publicado) com nota do motivo. SÓ
                                                              marcação no painel — desktop não muda. Recusa
                                                              cancelar vídeo já publicado (409). 3º estado além
                                                              de pendente/publicado. Colunas video_cancelado/
                                                              cancelado_em/cancelado_nota (bloco H idempotente
                                                              em _estrutura.php).
...
| `mega_pasta_logica` | **(Fase 1 MEGA)** Índice canônico das pastas lógicas do vídeo. | `UNIQUE (id_atividade, nome_pasta)` — duplicidade só dentro do mesmo canal. `nome_pasta` no formato `"NN - Titulo"` (montado por `mega_normalizar_nome_pasta`). Soft-delete (`ativo`); `desktop_criar_pasta.php` reativa se existir desativada. **Publicação:** `video_publicado`/`publicado_em` (bloco F). **Cancelamento (2026-07-17, bloco H):** `video_cancelado`/`cancelado_em`/`cancelado_nota` — 3º estado (pendente/publicado/cancelado). Só marcação no painel. |
| `pagamento_descontos` | **(2026-07-17)** Descontos avulsos de horas por usuário (abate do "A pagar"). | Colunas: `id_usuario`, `data_desconto`, `segundos_desconto`, `valor` (=segundos×valor_hora no momento), `motivo`. **NÃO trava tarefa nem reseta ciclo** (≠ Pagamentos) — separada de propósito pra apps desktop antigos, que leem só `Pagamentos` pra detectar ciclo novo, não enxergarem desconto como pagamento. Criada lazy via `pagamento_descontos_garantir_tabela()` em `commands/pagamentos/_descontos.php`. |
...
35. **Cold start do MEGAcmd: a PRIMEIRA chamada do dia falha sem ter rodado (v4.1.7).** Depois de ligar o PC o daemon `MEGAcmdServer.exe` está desligado. O `MEGAclient.exe` sobe o daemon **em background** e sai **imediatamente** com erro, sem executar o comando pedido: `MEGAcmd Server not running. Initiating in the background...` com `rc=-2` — que chega no Python como **`rc=4294967294`** (unsigned 32-bit). Sintoma pro usuário: ao abrir o app recém-ligado, faixa vermelha `Pastas MEGA não sincronizadas: Login no MEGA falhou: mega-login falhou (rc=4294967294)` e **botão "Declarar Tarefa" bloqueado** (`app/subtarefas.py::_aplicar_estado_mega_sync_na_ui`); clicar em "Atualizar MEGA" logo depois já funciona, porque aí o daemon subiu. **Não é antivírus nem instalação corrompida** (≠ armadilha #34) e **não é conexão** (≠ "Failed to access server" no troubleshooting) — é corrida de inicialização. **Fix:** `_run_mega` detecta via `_servidor_ainda_subindo(proc)` (`MARCADORES_SERVIDOR_SUBINDO`: `server not running`, `initiating in the background`, `initiating server`, `waiting for the server`) e repete o comando em degraus `ESPERAS_SERVIDOR_SUBINDO_SEG = (3.0, 5.0, 8.0)` — até 4 tentativas, ~16s no pior caso. **Repetir é seguro SÓ nesse caso** porque o comando comprovadamente não chegou ao servidor; qualquer outro `rc != 0` propaga na primeira tentativa, sem retry. **Não estender o retry pra `rc != 0` genérico** — `mega-put`/`mega-rm` não são idempotentes. O `mega-put` (upload) roda por `_executar_mega_put_streaming` (Popen próprio) e **não** passa por esse retry.
...
### "MEGAcmd Server not running" na abertura do app (RESOLVIDO em v4.1.7)

**Sintoma:** faixa vermelha `Pastas MEGA não sincronizadas: Login no MEGA falhou: mega-login falhou (rc=4294967294): MEGAcmd Server not running. Initiating in the background...` e botão "Declarar Tarefa" bloqueado, **só na primeira abertura depois de ligar o PC**. **Causa:** corrida de inicialização do daemon do MEGAcmd — o app não esperava o servidor subir (ver armadilha #35). **Não confundir** com "Failed to access server" (conexão) nem com instalação parcial (armadilha #34). **Workaround em versões < v4.1.7:** clicar em **Atualizar MEGA** ou reabrir o app — na segunda tentativa o daemon já está de pé. **Fix definitivo (v4.1.7+):** retry automático com espera em `_run_mega`.
