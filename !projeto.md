...
| `pyproject.toml` | Config de tooling: `ruff` (target py310, line-length 120), `mypy`, `pytest` (testpaths `tests/`). **`version = "2.2"` está desatualizado** vs `VERSAO_APLICACAO` em `app/config.py` (atualmente v4.0.3) — fonte da verdade da versão é sempre `app/config.py`. |
