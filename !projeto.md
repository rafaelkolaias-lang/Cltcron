...
| `mega-campos.php` | Página **MEGA · Campos de upload** (split de 2026-06-05): config de pasta raiz por canal + campos por usuário+canal (com `<select>` de Tipo e barra "Aplicar modelo") + **tabela CRUD de Modelos inline** (`#tbodyMegaModelos`: editar/excluir/+novo, substituiu os popups `window.prompt`). Também tem `<section id="abaMega">`. Acessada pelo submenu de "MEGA" no `_layout/topo.php`. |
